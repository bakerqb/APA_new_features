    ...
    def validateAllPlayersAvailableForAtLeastOneMatch(self, team: Team, matchNumber: int):
        for player in team.getPlayers():
            ...
            if not isPlayerAvailable:
                print(f"ERROR: {player.getPlayerName()} from team {team.getTeamName()} is unavailable for the rest of the matches")
                exit(1)
        ...
        while True:
            ...
            for matchSet, players in availableMatches.items():
                if len(matchSet) < len(players):
                    print(f"ERROR: {list(map(lambda player: player.getPlayerName(), players))} all need to play in matches {list(matchSet)}, which is not possible")
                    exit(1)

                if len(matchSet) == 0:
                    print(f"ERROR: {list(map(lambda player: player.getPlayerName(), players))} can't play any matches")
                    exit(1)
                if len(matchSet) == len(players):
        ...
        for matchSet, players in originalAvailableMatches.items():
            for matchNumber in restrictiveMatchSet:
                if matchNumber in matchSet:
                    del matchSet[matchNumber]
            newAvailableMatches[tuple(matchSet)] = players
    ...
    def validateAnyPlayersAvailableForMatch(self, team: Team, matchNumber):
        for matchNumberInQuestion in range(matchNumber, NUM_PLAYERMATCHES_IN_TEAMMATCH):
            ...
            if not someoneAvailableForMatch:
                print(f"ERROR: no one available to play match {matchNumberInQuestion + 1}")
                exit(1)